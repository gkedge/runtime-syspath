...
def persist_syspath(
    ...
    for index in range(0, len(sys_paths)):
        ...
        # python -m pytest ... or even when the Pycharm debugger is used.
...
def add_srcdirs_to_syspath(user_provided_project_dir: PurePath = None) -> None:
    ...
    if project_dir != Path.cwd():
        print(f"Searching for 'src' dirs from {project_dir.as_posix()}")

    prior_sys_path = sys.path.copy()

    all_src_dirs: Generator[Path, None, None] = project_dir.rglob("src")
    all_test_src_dirs: Generator[Path, None, None] = project_dir.rglob("tests/**/src")
